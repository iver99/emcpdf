...
//		optionXpath = "//ul[@id='oj-listbox-results-" + selectId + "']/li[" + (index + 1) + "]/div";
		...
		int listCount = driver.getElementCount("xpath=" + "//select[@id='" + selectId + "']/option");
		int i = 1;
		for (i=1; i<=listCount; i++) {
			if (optionId.equals(driver.getAttribute("xpath=(" + "//select[@id='" + selectId + "']/option" + ")[" + i + "]@id").trim()))
				break;
		}
		//get option's xpath generated by jet
		optionXpath = "//ul[@id='oj-listbox-results-" + selectId + "']/li[" + (i) + "]/div";
		return optionXpath;
	}
